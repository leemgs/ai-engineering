# AI 엔지니어링의 4가지 레이어 — Claude (클로드) 편

AI 시스템을 구축하고 고도화할 때 고려해야 하는 **4가지 핵심 레이어(프롬프트, 컨텍스트, 하네스, 루프)**의 개념과 앤트로픽(Anthropic)의 **클로드(Claude)**를 활용한 실제 구현 예시를 정리한 문서입니다.
...
| 레이어 | 클로드(Claude) 관점의 의미 | 비유 | 주요 역할 |
| :--- | :--- | :--- | :--- |
...
프롬프트 주변에 AI가 참고할 수 있는 **배경 정보, 제약 조건, 예시(Few-shot)** 등을 감싸주는 단계입니다. 클로드의 `System Prompt` 기능을 활용해 설정합니다.

* **개념:** AI에게 페르소나를 부여하고 가이드라인을 제공하여 답변의 일관성을 확보합니다.
* **실전 예시 (System Prompt 설정):**
    ```text
...
AI 모델이 서비스 내에서 안정적으로 작동하도록 돕는 **코드적인 안전장치(파이프라인)**입니다. 클로드 API를 파이썬(Python) 등의 코드로 감싸서 제어합니다.
...
* **실전 예시 (Python 가상 코드):**
    ...
    # 클로드 API 호출 (컨텍스트와 프롬프트 결합)
    response = client.messages.create(
        model="claude-sonnet-5",
        max_tokens=1000,
        system="너는 책 추천 봇이야. 답변은 반드시 JSON 형식 {'book': '제목', 'reason': '이유'}로만 해.",
    ...
    try:
        ...
        # 클로드가 형식을 어겼을 때 에러를 터뜨리지 않고 부드럽게 대처하는 로직
...
* **실전 예시 (Python 구현 코드):**
    ...
    # 1차 초안 생성
    draft_recommendation = ask_claude(system_context, user_complaint)

    while current_turn < max_turns:
        current_turn += 1
        print(f"🔄 [루프 {current_turn}회차] 클로드의 답변을 스스로 검증 중...")
        ...
        critic_result_raw = ask_claude("너는 엄격한 검수관이야. JSON으로만 답해.", critic_prompt)
        ...
        if critic_result["is_passed"]:
            ...
            draft_recommendation = ask_claude(system_context, fix_prompt)
...
## 🗂️ Claude Code 프로젝트 구조
최대 컨텍스트·모듈화·자율성을 위한 Claude Code의 프로젝트 설정 구조입니다.

![Claude Code 프로젝트 구조](../img/claude-code-proejct-structure.png)

---

## 🔎 Claude의 특징
- **긴 컨텍스트 처리와 안전성:** 대용량 문서 이해와 일관된 지침 준수(Instruction Following)에 강점을 가집니다.
- **Constitutional AI:** 자체 원칙 기반의 안전한 응답 성향으로, 하네스/루프 구조와 결합했을 때 예측 가능한 결과를 얻기 쉽습니다.
- **에이전트 친화적:** Tool Use, MCP(Model Context Protocol) 등 도구 연동 및 에이전트 구성에 최적화되어 있습니다.
