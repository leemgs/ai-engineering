# AI 엔지니어링의 4가지 레이어 — Gemini (제미나이) 편

AI 시스템을 구축하고 고도화할 때 고려해야 하는 **4가지 핵심 레이어(프롬프트, 컨텍스트, 하네스, 루프)**의 개념과 구글(Google)의 **제미나이(Gemini)**를 활용한 실제 구현 예시를 정리한 문서입니다.
...
| 레이어 | 제미나이(Gemini) 관점의 의미 | 비유 | 주요 역할 |
| :--- | :--- | :--- | :--- |
| **1. 프롬프트 (Prompt)** | 사용자가 입력하는 직접적인 명령어 | **씨앗** (핵심 알맹이) | AI에게 무엇을 묻고 요청할지 결정 |
| **2. 컨텍스트 (Context)** | System Instruction, 배경 지식, 제약 조건 | **토양** (자라나는 배경) | AI가 참고할 맥락과 출력 형식 지정 |
...
프롬프트 주변에 AI가 참고할 수 있는 **배경 정보, 제약 조건, 예시(Few-shot)** 등을 감싸주는 단계입니다. 제미나이의 `System Instruction` 기능을 활용해 설정합니다.

* **개념:** AI에게 페르소나를 부여하고 가이드라인을 제공하여 답변의 일관성을 확보합니다.
* **실전 예시 (System Instruction 설정):**
    ```text
...
AI 모델이 서비스 내에서 안정적으로 작동하도록 돕는 **코드적인 안전장치(파이프라인)**입니다. 제미나이 API를 파이썬(Python) 등의 코드로 감싸서 제어합니다.
...
* **실전 예시 (Python 가상 코드):**
    ...
    # 제미나이 API 호출 (컨텍스트와 프롬프트 결합)
    # response_mime_type을 지정해 JSON 출력을 강제할 수 있습니다.
    response = client.models.generate_content(
        model="gemini-2.5-flash",
        contents=user_input,
        config=types.GenerateContentConfig(
            system_instruction="너는 책 추천 봇이야. 답변은 반드시 JSON 형식 {'book': '제목', 'reason': '이유'}로만 해.",
    ...
    try:
        ...
        # 제미나이가 형식을 어겼을 때 에러를 터뜨리지 않고 부드럽게 대처하는 로직
...
* **실전 예시 (Python 구현 코드):**
    ...
    def ask_gemini(system_instruction, user_message):
        response = client.models.generate_content(
            model="gemini-2.5-flash",
    ...
    # 1차 초안 생성
    draft_recommendation = ask_gemini(system_context, user_complaint)

    while current_turn < max_turns:
        current_turn += 1
        print(f"🔄 [루프 {current_turn}회차] 제미나이의 답변을 스스로 검증 중...")
        ...
        critic_result_raw = ask_gemini("너는 엄격한 검수관이야. JSON으로만 답해.", critic_prompt)
        ...
        if critic_result["is_passed"]:
            ...
            draft_recommendation = ask_gemini(system_context, fix_prompt)
...
## 🗂️ Gemini CLI 프로젝트 구조
멀티모달·초장문 컨텍스트·구글 생태계 연동을 위한 Gemini CLI의 프로젝트 설정 구조입니다.

![Gemini CLI 프로젝트 구조](../img/gemini-cli-project-structure.png)

---

## 🔎 Gemini의 특징
- **멀티모달(Multimodal) 네이티브:** 텍스트뿐 아니라 이미지, 오디오, 비디오를 하나의 모델에서 함께 이해합니다.
- **초장문 컨텍스트:** 매우 긴 컨텍스트 윈도우를 지원해 대용량 문서/코드베이스 전체를 한 번에 다룰 수 있습니다.
- **구글 생태계 연동:** 구글 클라우드(Vertex AI), 워크스페이스 등과의 통합 및 구조화 출력(`response_schema`) 기능이 강력합니다.
